<template>
  <div class="verify-page">
      <el-card class="verify-box" title="验证邮箱">
          ...
          <p class="small-t">2. 将 <el-tag size="mini">no-reply@toolbee.cn</el-tag> 添加到白名单，重新发送</p>
          <div class="btn-row">
            <el-button type="primary" @click="sendMail" :disabled="!show">{{show ? '发送验证邮件' : `${count}s 后重新发送`}}</el-button>
          </div>
      </el-card>
...
export default {
  ...
  mounted() {
    const verifyCode = this.$route.query.code
